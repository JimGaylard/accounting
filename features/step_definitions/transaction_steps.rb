...
When(/^I create an unbalanced transaction$/) do
  trans = Accounting::Transaction.new()
  trans.entries = {
                  @sales => [:credit, 220.00],
                  @debtors => [:debit, 200.00]
...
Then(/^I should see no change in the balance of those accounts$/) do
  @sales.balance.should == 0
  @debtors.balance.should == 0
end
